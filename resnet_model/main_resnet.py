...
#read the training and val data 
    ...
    n = -1
    p = 0 
    add_train = '/data/home/dujw/code/view_classification/preprocessing_model/new_add_data_0220/Filtered/new_pwtdi_img_train_val_aug/train/'
    add_val= '/data/home/dujw/code/view_classification/preprocessing_model/new_add_data_0220/Filtered/new_pwtdi_img_train_val_aug/val/'
    for i in os.listdir('/data/home/dujw/code/view_classification/preprocessing_model/new_add_data_0220/Filtered/new_pwtdi_img_train_val_aug/train/'):
       # print(i)
        n = n +1
#	print(n)
        for u in os.listdir(add_train+i):
           p = p +1 
	   k=cv2.imread(add_train+i+'/'+u)
	   k = cv2.resize(k, (img_rows, img_cols))

	   X_train_new.append(k) 
           y_train_new.append(n)
           e = e + 1
    ...
    q= 0 
    z= -1
    for i in os.listdir(add_val):
        print(i)
        z = z +1
	print(z)
        for u in os.listdir(add_val+i):
            a=cv2.imread(add_val+i+'/'+u)
	    a = cv2.resize(a, (img_rows,img_cols))
	    print(a.shape)
	    X_test_new.append(a)
            y_test_new.append(z)
            q = q + 1

    a = range(len(X_train_new))
    random.shuffle(a)
    X_train = [X_train_new[idx] for idx in a]
    y_train = [y_train_new[idx] for idx in a]
    a = range(len(X_test_new))
    random.shuffle(a)
    X_test = [X_test_new[idx] for idx in a]
    y_test = [y_test_new[idx] for idx in a]
